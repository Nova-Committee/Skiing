...
public class MultiTextureEntityRenderer extends EntityRenderer<AbstractMultiTextureEntity> {
    ...
    public @NotNull ResourceLocation getTextureLocation(AbstractMultiTextureEntity entity) {
        ...
        try {
            ResourceLocation sprite = Minecraft.getInstance().getBlockRenderer().getBlockModel(block.defaultBlockState()).getQuads(block.defaultBlockState(), Direction.NORTH, RandomSource.create()).get(0).getSprite().contents().name();
